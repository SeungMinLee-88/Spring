...
<tr>
      ...
      style="background: #FF0000;boder:1px solid black;text-decoration:none;text-decoration:none;font-weight: bold;">
      TODAY
		</a>
		</td>
</tr>
       ...
       <tr>
		<td align="center" >
                    ...
                    <%if(month > 0 && month+1 > curmonth ){ %>
                     <a id="booktag" href="./RoomBookModifyCalendar?year=<%=year%>&amp;month=<%=month-1%>&amp;roomnum=<%=roomnum%>&amp;bookdate=<%=bookdate%>">
                           <b>prev</b><!-- 이전달 -->
                    </a>
                    <%} else {%>
                    <%} %>
...
%>
		...
		%>
			<br>
			<a id="booktag" id="booktag" href="#" class="input"
			onclick="javascript:bookdateinput('<%=inputDate%>','<%=book_time_string%>');">
			<font color="black" style="font-weight:bold;">
			<%if(inputDate.equals(bookdate)){ %>
			수정
			<%}else{ %>
			예약
			<%} %>
