...
<body>
<div id="reserve_list" style="margin-left:200px;margin-top: 40px;position:relative;display: inline-block;text-align: center;">
...
<td height="60px" align="center" >
       <table width="1000px" border="0" cellspacing="0" cellpadding="0" align="center">
		...
		<td align="center" >
                   ...
                   <a id="booktag" href="./AdminReserveList?year=<%=year%>&amp;month=<%=month-1%>&amp;roomnum=<%=roomnum%>">
                          <b>prev</b><!-- 이전달 -->
                    </a>
                    <%} else {%>
                    <%} %>
...
<c:set target="${mapTest}" property="H" value="17:00~18:00" />
		<c:choose> 
			...
			<tr>
			<td width="700" height="300" align="center">등록된 예약이 없습니다.</td>
			</tr>
			</c:when>	
		<c:otherwise> 	
	...
	<a href="./DeleteBook?book_num=${item.BOOK_NUM}&page=<%=nowpage%>&month=<%=month%>&amp;roomnum=<%=roomnum%>"
	style="text-decoration:none;">
	삭제</a>
	&nbsp;
	<a href="./ModifyBook?book_num=${item.BOOK_NUM}&page=<%=nowpage%>&month=<%=month%>&amp;roomnum=<%=roomnum%>"
	style="text-decoration:none;">
	수정
	</a>
	</td>
...
<table align="center" width="900px" style="margin-top:15px;">
	...
	<div id="paging">		
		...
		if (count>0) {
			if (startPage>10) { %>
			<a href="AdminReserveList?year=<%=year%>&amp;month=<%=month%>&amp;roomnum=<%=roomnum%>&page=<%=startPage-1%>" style="text-decoration:none;">[이전]</a>
			<% }
			for (int i=startPage;i<=endPage ; i++) { 
				...
				<% } else { 
				%>  
					<a href="AdminReserveList?month=<%=month%>&amp;roomnum=<%=roomnum%>&page=<%=i%>" style="text-decoration:none;">[<%=i%>]</a>
					<%
				}
			}
			if (endPage<pageCount) { %>
			<a href="AdminReserveList?year=<%=year%>&amp;month=<%=month%>&amp;roomnum=<%=roomnum%>&page=<%=endPage+1%>" style="text-decoration:none;">[다음]</a>
			<%
			}
		}
		%>			
