...
<style type="text/css">
.Videolist, .Videolist th, .Videolist td{
	font-size:15px;
...
%>	
	...
	<a href="./VideoWrite" onclick ="admincheck()"><img src="./image/videoimage/write.png" width="70px"></a>
	</td>
	</tr>
	</table>
<%
...
%>
<form name="searchform" action="VideoList" method="post">
...
<table border="0" width="400px"  style="margin-left:350px;margin-bottom:50px;">
	<tr>
		...
		<input type="button" onclick="javascript:Videoserarch();" value="검색">
		</td>
	</tr>
</table>

<table class="Videolist" align="center">
...
<c:choose> 
	<c:when test="${requestScope.topitemList[0].VIDEO_NUM!=null}">	
	<c:forEach var="item2" items="${requestScope.topitemList}">
	<tr style="background: #eef8ff;">
		<td align="center">
		<img id="topimage" src="./image/videoimage/video.jpg" width="30px" >
		<input type="hidden" name="board_num" value="${item2.VIDEO_NUM}">
		</td>
		<td> 
		<div id="Videolist">
	 	<a href="./VideoView?video_number=${item2.VIDEO_NUM}&page=<%=nowpage%>&searchtype=<%=searchtype%>&searchkeyword=<%=searchkeyword%>"> ${item2.VIDEO_TITLE}</a>
		</div>
		</td>
		<td align="center">${item2.VIDEO_WRITER}
		</td>
		<td align="center">${item2.VIDEO_HIT}
		</td>
...
<c:choose> 
		<c:when test="${requestScope.itemList[0].VIDEO_NUM==null}">
	<tr  height="50px">
	...
	<tr>
		...
		<input type="hidden" name="board_num" value="${item.VIDEO_NUM}">
		</td>
		<td> 
	<div id="Videolist">
 	<a href="./VideoView?video_number=${item.VIDEO_NUM}&page=<%=nowpage%>&searchtype=<%=searchtype%>&searchkeyword=<%=searchkeyword%>">
 	${item.VIDEO_TITLE}
 	</a>
	</div>
		</td>
		<td align="center">${item.VIDEO_WRITER}
		</td>
		<td align="center">${item.VIDEO_HIT}
		</td>
...
<table align="center" width="600px" style="margin-top:15px;">
	...
	<div id="paging">		
		...
		if (count>0) {
			if (startPage>10) { %>
			<a href="VideoList?page=<%=startPage-1%>&searchtype=<%=searchtype%>&searchkeyword=<%=searchkeyword%>">[이전]</a>
			<% }
			for (int i=startPage;i<=endPage ; i++) { 
				...
				<% } else { 
				%>  
					<a href="VideoList?page=<%=i%>&searchtype=<%=searchtype%>&searchkeyword=<%=searchkeyword%>" style="text-decoration:none;">
					<font color="black">[<%=i%>]</font></a>
					<%
				}
			}
			if (endPage<pageCount) { %>
			<a href="VideoList?page=<%=endPage+1%>&searchtype=<%=searchtype%>&searchkeyword=<%=searchkeyword%>">[다음]</a>
			<%
			}
		}
		%>			
